...
function Message() {
  const messageArray = [
    ...
    },
    {
      firstName: "Sarah",
      lastname: "Mahanotrah",
      msg: "I'm blown away by the quality and style of the clothes I received from CEE.SHOP .",
      verfied: true,
      rating: 5,
    },
  ];
  ...
  const handleNext = () => {
    ...
    setCurrentIndex((prevIndex) => (prevIndex + 1) % messageArray.length);
    // }
  };
  ...
  return (
    <div className="px-5 mt-12">
      <div className="flex">
        <div className="font-black text-5xl  w-5/6 md:w-11/12">OUR HAPPY CUSTOMERS</div>
        <div className="flex items-end w-1/6 justify-around md:w-1/12 ">
          <button
            className=""
            onClick={handlePrev}
      ...
      <div className="">
        <div className="flex md:hidden py-5 overflow-x-auto">
        ...
        <div className="hidden md:flex py-5 justify-around overflow-x-auto">
          {Array.from({ length: 3 }).map((_, i) => {
            const message = messageArray[currentIndex + i];
            if (!message) return null; // Skip rendering if no message exists at this index

            return (
              <div
                key={currentIndex + i}
                className="border-black border-opacity-10 border-2 rounded-2xl px-5 py-5"
              >
                {renderRating(message.rating)}
                <div className="flex items-center font-bold text-2xl">
                  {message.firstName} {message.lastname?.[0]}
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 16 16"
                    fill="currentColor"
                ...
                <div className="text-opacity-60 text-black">{message.msg}</div>
              </div>
            );
          })}
        </div>
